...
Potree.PathControls = class PathControls extends THREE.EventDispatcher {
    constructor(viewer) {
        super();
    ...
    update(delta) {
...

        {
            if (this.path !== null) {
                const deltaPosition =
                    this.translationDelta.y * delta / this.pathLength;
...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher{
	...
	getControls (navigationMode) {
		...
		} else if (navigationMode === Potree.PathControls) {
	...
	createControls () {
		...
		{
			this.pathControls = new Potree.PathControls(this);
